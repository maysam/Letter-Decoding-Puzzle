...
function drawPuzzle() {
	...
	for(var j=0; j<ROWS; j++) {
		...
		for(var i=0; i<COLUMNS; i++) {
			...
			has_guess_1 = (word1 && guesses[word1.index] != undefined && guesses[word1.index].length > 1);
			//	there are guesses for word 2
			has_guess_2 = (word2 && guesses[word2.index] != undefined && guesses[word2.index].length > 1);
			index1 = index2 = -1
        ...
        if (word1) {
	        ...
	        if (newWord && newWord != word) { //  something new
						word = newWord;
						word.index = current_index = current_index = _.indexOf(wordList,word); //  fix for removing single words
    ...
    if (word) {
      	...
      	var top = PUZZLE_TOP+SIZE*(ROWS+.5 + j);
				...
				if ( i < guesses[current_index].length-1 && k == word.length+1 ) {
					if (word.hint.join() != word.data.join()) {
						...
						events.push([left+2, top+2, SIZE-10, SIZE-10, function(current_index, actual_i){ return function(){
		...
		if (max > 5) {
			...
			events.push([x_left + SIZE - 10, x_top , SIZE, SIZE*5, function(x_top, max_scroll) { return function (x, y) {
...
function drawAlphaBar(group, offset) {
  ...
  for (var i_counter=0; i_counter<5; i_counter++) {
    ...
    if ( i == 4 && group == 5) {
  	...
  	if(!word.hint[offset])
	  	if ( i == 4 && group == 5) {
	    	events.push(plan (left, top, SIZE/2, SIZE/2, selected_data[i], offset, i, group))
	    	events.push(plan (left+SIZE/2, top +SIZE/2 , SIZE/2, SIZE/2, selected_data[i+1], offset, i, group))
	  	} else {
	    	events.push(plan (left, top, SIZE, SIZE, selected_data[i], offset, i, group))
	  	}
  }
}
